...
class User 
{
	...
	function __construct($user,$display,$pass,$email, $is_admin)
	{
		...
		$this->is_admin = sanitize($is_admin);
	...
	public function userCakeAddUser()
	{
		...
		if($this->status)
		{
			...
			if(!$this->mail_failure)
			{
				...
				$stmt->bind_param("sssssii", $this->username, $this->displayname, $secure_pass, $this->clean_email, $this->activation_token, $this->user_active, $this->is_admin);
				$stmt->execute();
				$inserted_id = $mysqli->insert_id;
				$stmt->close();
				
				//Insert default permission into matches table home page permission with id = 1
